...
@Component({
  selector: 'app-tablatasladoscaja',
  templateUrl: './tablatasladoscaja.component.html',
  styleUrls: ['./tablatasladoscaja.component.scss', '../../../style.scss']
})
export class TablatasladoscajaComponent implements OnInit {
  ...
  constructor(private _generalService: GeneralService,
              ...
              private _trasladoService:TrasladocajaService,
  ...
  search_origen = (text$: Observable<string>) =>
    ...
    .pipe(
      ...
      switchMap( term => term.length < 4 ? [] :
        this._funcionarioService.FiltrarFuncionariosXNombre(term)
        .map(response => response)
        .do(data => data)
      )
    );
  ...
  ConsultaFiltrada(paginacion:boolean = false) {
    ...
    this.Cargando = true;
    this._trasladoService.GetListaTrasladoCaja(p).subscribe((data:any) => {
      if (data.codigo == 'success') {
